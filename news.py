...
def parse_article(url: str) -> Optional[dict]:
    ...
    return {"url": url, "date": date, "category": category, "title": title, "text": text}
...
def main():
    ...
    if not os.path.exists(url_path):
        logger.error(f"❌ Không tìm thấy {url_path} — hãy chạy 1_url_collector.py trước!")
        return
        ...
        for future in as_completed(futures):
            try:
                ...
                if result:
                    ...
                    if done % 100 == 0:
                        logger.info(f"   ✅ {done:,}/{len(pending):,} bài | ❌ {failed} lỗi")
                else:
                    failed += 1
            except Exception:
                failed += 1
